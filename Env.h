...
#undef c
	...
	struct RunTimeEnvData {
		...
		lli currentFunction;
		vector<SA::Function>* functionListPtr;
	};
	...
	lli FindFunctionIndex(string functionName, vector<SA::Function>& functionList) {
		lli index = -1;
	...
	vector<SA::Function> MapLabels(vector<SA::Function>& functionList_argument) {
		vector<SA::Function> functionList = functionList_argument;
		struct Name_Address {
			lli labelName;
			lli labelAddress;
		};
	...
	fnPtr(void, executeFunctionPtr, vector<SA::Function>& functionList, lli functionIndex);
	lli ExecuteInstruction(lli inst_idx, vector<Iasm::Instruction>& instructionList,
		vector<SA::Function>& functionList, RunTimeEnvData* rted)
	{
...
//					}	
		...
		case WRITE_ARG: {
			...
			stack.Get<ulli>(beginFunctionPtr - (argIndex * 4) - 4) = arg;
			//                                    ^ hard coded BAD
			break;
		}
		case MOV:{
...
#undef c
	...
	void ExecuteFunction(vector<SA::Function>& functionList, lli functionIndex) {
		RunTimeEnvData rted;
	...
	void Execute(vector<SA::Function> functionList) {
