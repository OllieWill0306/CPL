...
namespace IDE {
	...
	#pragma comment(lib, "NOT_MY_CODE/Libs/glfw/lib-vc2022/glfw3.lib")
	#pragma comment(lib, "NOT_MY_CODE/Libs/glew/lib/Release/x64/glew32s.lib")
	...
	#include "NOT_MY_CODE/Libs/glew/include/GL/glew.h"
	#include "NOT_MY_CODE/Libs/glfw/include/GLFW/glfw3.h"
	...
	int2 RenderText(string text, int x, int y, float scale = 4.f, float r = 1.f, float g = 1.f, float b = 1.f) {
		...
		loop(c, 0, text.size()) {
			...
			case '\n':
				...
				xPos = -1;//because gets incriented at end of loop
				lineCount++;
				break;
			case '\t':
...
//#endif
	...
	vector<vector<bool>> ByteCodeSetBreakPoints(vector<AST::Function>* functionList_ptr) {
		vector<vector<bool>> breakPointList;
		vector<AST::Function>& functionList = $(functionList_ptr);
